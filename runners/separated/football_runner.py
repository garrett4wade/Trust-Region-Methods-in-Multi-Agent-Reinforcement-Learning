    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                if train_ep_cnt > 0:
                    train_env_info = dict(train_episode_length=train_ep_length / train_ep_cnt,
                                          train_episode_return=train_ep_ret / train_ep_cnt)
                    print("Average training episode return is {}, episode length is {}.".format(
                        train_ep_ret / train_ep_cnt, train_ep_length / train_ep_cnt))
                    if self.all_args.use_wandb:
                        wandb.log(train_env_info, step=total_num_steps)
                    else:
                        self.writter.add_scalars("train_env_info", train_env_info, total_num_steps)
    ...
    def eval(self, total_num_steps):
        ...
        while True:
            ...
            if eval_episode >= self.all_args.eval_episodes:
                ...
                print("Eval average episode reward is {}.".format(sum(eval_ret) / len(eval_ret)))
                break
