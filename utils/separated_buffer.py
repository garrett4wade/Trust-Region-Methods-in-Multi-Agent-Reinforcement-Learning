...
class SeparatedReplayBuffer(object):
    ...
    def feed_forward_generator(self,
        ...
        if self.value_after_update is not None:
            new_value = self.value_after_update.reshape(
                -1, self.value_after_update.shape[-1])
        else:
            new_value = None
        if self.actor_output_after_update is not None:
            new_actor_output = self.actor_output_after_update.reshape(
                -1, self.actor_output_after_update.shape[-1])
        else:
    ...
    def recurrent_generator(self, advantages, num_mini_batch,
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                if self.value_after_update is not None:
                    new_value_batch.append(new_value[ind:ind +
                                                     data_chunk_length])
                if self.actor_output_after_update is not None:
                    new_actor_output_batch.append(
                        new_actor_output[ind:ind + data_chunk_length])
