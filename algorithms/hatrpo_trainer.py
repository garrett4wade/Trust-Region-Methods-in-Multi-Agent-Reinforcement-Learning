...
class HATRPO():
    ...
    def kl_divergence(self, obs, rnn_states, action, masks, available_actions,
                      active_masks, new_actor, old_actor):
        _, _, mu, std, probs = new_actor.evaluate_actions(
            obs, rnn_states, action, masks, available_actions, active_masks)
        _, _, mu_old, std_old, probs_old = old_actor.evaluate_actions(
            obs, rnn_states, action, masks, available_actions, active_masks)
        if mu.grad_fn == None:
            ...
            kl = logstd - logstd_old + (std_old.pow(2) +
                                        (mu_old - mu).pow(2)) / (
                                            2.0 * std.pow(2)) - 0.5
    ...
    def conjugate_gradient(self,
        ...
        for i in range(nsteps):
            _Avp = self.fisher_vector_product(actor, obs, rnn_states, action,
    ...
    def fisher_vector_product(self, actor, obs, rnn_states, action, masks,
                              available_actions, active_masks, p):
        p.detach()
        kl = self.kl_divergence(obs,
                                rnn_states,
                                action,
                                masks,
                                available_actions,
        ...
        kl_hessian_p = torch.autograd.grad(kl_grad_p,
                                           actor.parameters(),
                                           allow_unused=True)
    ...
    def trpo_update(self, sample, update_actor=True):
        ...
        values, action_log_probs, dist_entropy, action_mu, action_std, _ = self.policy.evaluate_actions(
        ...
        if self._use_policy_active_masks:
            ...
            loss = torch.sum(ratio * factor_batch * adv_targ,
                             dim=-1,
                             keepdim=True).mean()
        ...
        fvp = self.fisher_vector_product(self.policy.actor, obs_batch,
                                         rnn_states_batch, actions_batch,
                                         masks_batch, available_actions_batch,
                                         active_masks_batch, step_dir)
        shs = 0.5 * (step_dir * fvp).sum(0, keepdim=True)
        step_size = 1 / torch.sqrt(shs / self.kl_threshold)[0]
        full_step = step_size * step_dir

        old_actor = Actor(self.policy.args, self.policy.obs_space,
                          self.policy.act_space, self.device)
        ...
        for i in range(self.ls_step):
            ...
            if self._use_policy_active_masks:
                ...
                new_loss = torch.sum(ratio * factor_batch * adv_targ,
                                     dim=-1,
                                     keepdim=True).mean()
            ...
            if kl < self.kl_threshold and (
                    loss_improve / expected_improve
            ) > self.accept_ratio and loss_improve.item() > 0:
                flag = True
                break
